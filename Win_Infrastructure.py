...
class Win_Infrastructure:
    ...
    def open_inf(self, root):
        ...
        label_room = Label(win, text = "Room" ,font=('calibre',20, 'bold') )
        label_room.pack(pady= 5)
        ...
        for record in data:
            if count % 2 == 0:
                my_tree1.insert(parent='', index='end', iid= count, text='', values=(record[0], record[1] ), tags= ("evenrow",))              
            else:
                my_tree1.insert(parent='', index='end', iid= count, text='', values=(record[0], record[1] ), tags= ("oddrow",))   
            count += 1
        ...
        def add_record():
            global count
            if count % 2 == 0:
                my_tree1.insert(parent='', index='end', iid= count, text='', values=(count+1, name_box.get()), tags= "evenrow",)
                ...
                my_tree1.insert(parent='', index='end', iid= count, text='', values=(count+1, name_box.get(), dob_box.get()), tags = "oddrow",)
